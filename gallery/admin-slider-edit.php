...
<body>
  <div class="container admin-friendly">
    ...
    <form method="post" action="/gallery/slider-save.php" enctype="multipart/form-data">
      ...
      <div class="control-group"><label for="display-order">Display order</label><input id="display-order" type="number" min="0" step="1" name="displayOrder" value="<?= (int) ($slide['displayOrder'] ?? 0) ?>"><p class="help-block">Lower numbers appear first. You can also reorder from the slider list.</p></div>
      <div class="control-group"><label><input type="checkbox" name="active" value="1"<?= !empty($slide['active']) ? ' checked' : '' ?>> Show on homepage</label><p class="help-block">Leave unchecked to keep this slide saved but hidden from the homepage.</p></div>
