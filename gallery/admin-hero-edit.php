...
<body>
  <div class="container admin-friendly">
    ...
    <form method="post" action="/gallery/hero-save.php" enctype="multipart/form-data">
      ...
      <div class="control-group"><label for="hero-title">Title</label><input id="hero-title" type="text" name="title" required value="<?= htmlspecialchars((string) ($hero['title'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"><p class="help-block">Shown as the main heading on the homepage panel and hero page.</p></div>
      ...
      <div class="control-group"><label><input type="checkbox" name="visible" value="1"<?= !empty($hero['visible']) ? ' checked' : '' ?>> Display on homepage</label><p class="help-block">Leave unchecked to save this as a hidden draft.</p></div>
      <div class="control-group"><label for="display-order">Display order</label><input id="display-order" type="number" min="0" step="1" name="displayOrder" value="<?= (int) ($hero['displayOrder'] ?? 0) ?>"><p class="help-block">Lower numbers appear first. You can also reorder from the hero list.</p></div>
      <div class="control-group"><label for="visual-style">Panel style</label><select id="visual-style" name="visualStyle"><option value="info"<?= ($hero['visualStyle'] ?? 'info') === 'info' ? ' selected' : '' ?>>Info (blue)</option><option value="success"<?= ($hero['visualStyle'] ?? '') === 'success' ? ' selected' : '' ?>>Success (green)</option><option value="neutral"<?= ($hero['visualStyle'] ?? '') === 'neutral' ? ' selected' : '' ?>>Neutral</option></select><p class="help-block">Sets the panel colour on the homepage and hero page.</p></div>
      ...
      <div class="control-group"><label for="image-alt">Image alt text</label><input id="image-alt" type="text" name="imageAlt" value="<?= htmlspecialchars((string) ($hero['imageAlt'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"><p class="help-block">Describes the image for screen readers; defaults to the title if left blank.</p></div>
      <div class="control-group"><label for="exhibition-id">Related exhibition</label><select id="exhibition-id" name="exhibitionId"><option value="">None</option><?php foreach ($exhibitions as $exhibition): ?><option value="<?= (int) $exhibition['id'] ?>"<?= (string) ($hero['exhibitionId'] ?? '') === (string) $exhibition['id'] ? ' selected' : '' ?>><?= htmlspecialchars($exhibition['title'], ENT_QUOTES, 'UTF-8') ?></option><?php endforeach; ?></select><p class="help-block">Optional. Links this hero to an exhibition for reference only.</p></div>
      <div class="hero-editor-layout">
        <div><label for="hero-body">Hero content</label><p class="help-block">Formatting: <code>#</code> main heading, <code>##</code> section heading, <code>###</code>-<code>######</code> smaller headings, <code>**bold**</code>, <code>*italic*</code>, blank lines for paragraphs, <code>- list items</code>, and <code>[link text](https://example.com)</code>. Use the title field as the page's main heading; normally begin content headings at <code>##</code>.</p><textarea id="hero-body" name="body" rows="14"><?= htmlspecialchars((string) ($hero['body'] ?? ''), ENT_QUOTES, 'UTF-8') ?></textarea></div>
        <div>
          <span class="admin-preview-label">Real-time preview (matches the homepage width and colours)</span>
          <div class="admin-preview-frame">
            <article id="hero-preview" class="alert alert-info home-hero-panel" aria-live="polite">
              <h2 id="hero-preview-title"></h2>
              <img id="hero-preview-image" src="<?= htmlspecialchars((string) ($hero['image'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" alt="" <?= empty($hero['image']) ? 'hidden' : '' ?>>
              <div id="hero-preview-body" class="home-hero-body"></div>
            </article>
          </div>
        </div>
      </div>
...
(function () {
  ...
  function update() {
    ...
    previewBody.innerHTML = render(bodyInput.value);
    panel.className = 'alert alert-' + (styleSelect.value || 'info') + ' home-hero-panel';
  }
